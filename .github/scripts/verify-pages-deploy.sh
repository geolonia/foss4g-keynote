...
#
#   PAGE_URL  サイトルートの URL（末尾スラッシュあり／なしどちらでも可）
#   SUBPATH   検証するページのサイトルートからの相対パス（例 "ja/"・"post/"。
#             省略時はルートページ）。アセットはサイトルート配下に集約されるため、
#             エントリ JS の取得は SUBPATH に依らず ${PAGE_URL}/${EXPECTED} で行う。
#   EXPECTED  配信されているべきエントリ JS のパス（例 assets/main-Bo3msxiW.js）
...
entry_of() {
  ...
  # main-/post- が現行（vite.config.ts の input: main/ja/post。/ と /ja/ は
  # main チャンクを共有）。index- は旧 livedeck 命名で、過去の配信物が
  # 残っている場合の診断表示のために残す。
  printf '%s' "$1" | grep -oE 'assets/(main|post|index)-[A-Za-z0-9_-]+\.js' | head -n 1 || true
}

kind=""

echo "expecting ${EXPECTED} at ${page}"
...
for attempt in $(seq 1 "$attempts"); do
  ...
  if [ -n "$matched" ]; then
    ...
    # アセットはサイトルート配下（/assets/…）なので base 起点で取得する。
    if fetch "${base}/${EXPECTED}" > /dev/null; then
      echo "OK: ${page} は ${EXPECTED} を配信しています（試行 ${attempt} 回目）"
      exit 0
    fi
    kind="js_missing"
...
if [ "$kind" = "js_missing" ]; then
  echo "::error::${page} の HTML は ${EXPECTED} を指していますが、その JS 本体を取得できません。アップロードした成果物が不完全な可能性があります。"
  exit 1
fi

origin=$(probe_origin "${page}?cachebust=${GITHUB_RUN_ID:-local}-final") || origin=""
origin_entry=$(entry_of "$origin")
case "$origin" in
  *"$EXPECTED"*)
    echo "::error::オリジンには ${EXPECTED} が反映済みですが、${page} ではまだ配信されていません（CDN のキャッシュが残っている可能性）。"
    ;;
  *)
    echo "::error::${page} が ${EXPECTED} を配信していません（オリジンの配信中: ${origin_entry:-取得失敗}）。GitHub Pages への公開自体が反映されていない可能性があります。"
    ;;
esac
exit 1
