...
2. [ ] **「→」でスライド2まで送る**。右下に地図の小窓が現れ、ステータスが「n 件（+仕込み m 件）」の件数表示になることを確認する
   - ★件数表示だけではWS接続の証にならない（WS接続に失敗しても履歴取得のフォールバックで件数は表示される実装）。**必ず次の3の生死確認まで行うこと**
   - 「読み込み中…」のまま止まる／エラー表示なら、この時点でのみ再読込してリトライしてよい（開演前なら枠は空いている）
3. [ ] **WS接続の生死を直接確認する**。ブラウザのDevToolsを開き、いずれかで確認:
   - Network タブ → 「WS」でフィルタ → WebSocket接続がステータス101で存在し、切断されていないこと
   - Console タブ → `[keynoteMap] ws` の警告が**出ていない**こと（この警告が出ていればWS不通＝履歴フォールバック表示であり、講演中の投稿が地図に反映されない）
   - WS不通だった場合は再読込してリトライ（開演前のみ可）。確認後はDevToolsを閉じてよい
4. [ ] **「←」でスライド1（QR）へ戻す**。地図は隠れるが接続は保たれる
5. [ ] フルスクリーン化など画面まわりの操作は開演前に済ませる
6. [ ] 予備テザリング端末を使う場合（下記④参照）: 予備端末でも同じ予熱（スライド2まで送る→件数表示＋WS生死確認→スライド1へ戻す）を済ませて待機
